...
const modalBackdrop = reviewModal ? reviewModal.querySelector(".modal-backdrop") : null;
...
reviewModal.addEventListener("click", (event) => {
  if (event.target === reviewModal || event.target.classList.contains("modal-backdrop")) {
    closeReview();
  }
});
...
function renderDashboard() {
  const streak = String(dashboardState.streak);
  streakValue.textContent = streak;
  const sv2 = document.getElementById("streak-value2");
  if (sv2) sv2.textContent = streak;
  ...
  if (ring) {
    const maxStreak = 30;
    const pct = Math.min(dashboardState.streak / maxStreak, 1);
...
function renderLibrary() {
  ...
  entries.forEach((item) => {
    ...
    li.className = `memo-card${isDue(item) ? " is-due" : ""}`;
...
function buildCardTemplate(item) {
  ...
  const tags = [];
  tags.push(`<span class="tag ${due ? "due" : ""}">${due ? "⚡ מוכן עכשיו" : "⏳ מתוזמן"}</span>`);
  tags.push(`<span class="tag">${item.type === "qa" ? "🧠 Learning" : "🔔 Reminder"}</span>`);
  if (item.type === "qa") {
    tags.push(`<span class="tag level-${clampLevel(item.level)}">רמה ${clampLevel(item.level)}</span>`);
    tags.push(`<span class="tag">ידע ${formatScore(item.score)}</span>`);
  }

  const inlineGrades = item.type === "qa" && due
    ? `<div class="inline-grades">
        <button class="grade-btn again sm-btn" type="button" data-action="grade" data-grade-value="again" data-id="${item.id}">
          <span class="grade-emoji">😰</span><span>לא ידעתי</span><span class="grade-time">שעה</span>
        </button>
        <button class="grade-btn hard sm-btn" type="button" data-action="grade" data-grade-value="hard" data-id="${item.id}">
          <span class="grade-emoji">😓</span><span>קשה</span><span class="grade-time">יום</span>
        </button>
        <button class="grade-btn good sm-btn" type="button" data-action="grade" data-grade-value="good" data-id="${item.id}">
          <span class="grade-emoji">🙂</span><span>טוב</span><span class="grade-time">3 ימים</span>
        </button>
        <button class="grade-btn easy sm-btn" type="button" data-action="grade" data-grade-value="easy" data-id="${item.id}">
          <span class="grade-emoji">😎</span><span>קל</span><span class="grade-time">שבוע</span>
        </button>
      </div>` : "";
  ...
  if (item.type === "qa") {
    primaryAction = due
      ? `<button class="sm-btn review" type="button" data-action="review" data-id="${item.id}">ענה</button>`
      : `<button class="sm-btn wait" type="button" disabled>ממתין</button>`;
  } else {
    primaryAction = `<button class="sm-btn wait" type="button" disabled>${due ? "מופיע עכשיו" : "ממתין"}</button>`;
  }
  ...
  return `
    <div class="card-row">
      ...
      <div class="card-actions-row">
        <button class="sm-btn edit" type="button" data-action="edit" data-id="${item.id}">עריכה</button>
        <button class="sm-btn del" type="button" data-action="delete" data-id="${item.id}">מחיקה</button>
      </div>
    </div>
    <p class="card-question"></p>
    ${inlineGrades}
    <div class="card-row">
      <span class="card-meta">⏰ ${formatNextReview(item.nextReviewDate)}</span>
      ${primaryAction}
    </div>
  `;
}
